...
b8 vulkan_device_create(vulkan_context* context){
    ...
    KINFO("Createing logical device...");

    // NOTE: Do not create additiona queues for shared indices.
    ...
    if(transfer_shares_present_queue){
        KINFO("Transfer queue shares same Index as Present Queue");
        index_count--;
    }
    ...
    for(u32 i=0; i<index_count; ++i){
        ...
        // // TODO: Enable this for future enhancment.
    ...
    // Depricated and ignored, so pass nothing
...
b8 select_physical_device(vulkan_context* context){
    ...
    for (u32 i = 0; i < physical_device_count; ++i)
    {
        ...
        requirements.minimum_heap_size = 4.5;
        ...
        if(result){
            ...
            context->device.transfer_queue_index    = queue_info.present_family_index;
...
b8 physical_device_meets_requirements(
    ...
    vulkan_swapchain_support_info*              out_swapchain_support)
{
    // Evaluate device propertiest to determine if it meets the needs of our application
    ...
    if(
        ...
        if(out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1){
            ...
            KINFO("Required swapchain support not resent, skipping device.");
            return FALSE;
        }   
        ...
        if(requirements->device_extension_names){
            ...
            if (available_extension_count != 0){
                ...
                for(u32 i=0; i<required_extension_count; ++i){
                    ...
                    if(!found){
                        KINFO("Required extension not found: %s, skippping device", requirements->device_extension_names[i]);
            ...
            // My memory requirements for my card!!!!!---------------------------------------------
            if(requirements->minimum_heap_size != 0 ){
                u32 heaps_count = mem_properties->memoryHeapCount;
                for(u32 i = 0; i < heaps_count; ++i){
                    f32 heap_size = mem_properties->memoryHeaps[i].size/1024.0f/1024.0f/1024.0f;
                    if((mem_properties->memoryHeaps[i].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) == VK_MEMORY_HEAP_DEVICE_LOCAL_BIT){
                        if(heap_size < requirements->minimum_heap_size){
                            KINFO("Device does not support minimum VRam requirements...");
                            return FALSE;
                        }else{
                            KINFO("Device meets minimum VRam requirements...");
                            KINFO("Device VRam in Gib: %.2f",heap_size);
                            break;
                        }
                    }else{
                        continue;
