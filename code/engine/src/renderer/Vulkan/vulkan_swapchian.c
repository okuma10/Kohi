...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchian);
...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    u32*                out_image_index)
    {
        ...
        if(result == VK_ERROR_OUT_OF_DATE_KHR){
            ...
            KFATAL("Failed to acquire swapchian image!");
            return FALSE;
        }
...
void vulkan_swapchain_present(
    ...
    {
        // Return the image to the swapchaing for presentation.
        ...
        if(result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR){
            // Swapchain is out of date, suboptimal or a framebuffer resize has occurred. Trigger swapchaing recreation.
            vulkan_swapchain_recreate(context, context->framebuffer_width, context->framebuffer_height, swapchain);
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain){
    ...
    //      Create depth image and it's view
    vulkan_image_create(
        context,
        VK_IMAGE_TYPE_2D,
    ...
    KINFO("Swapchain create successfully");
}
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain){
    ...
    // Only destroy the views, not the images, since those are owned by the swapchain and thus
